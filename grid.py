...
class Pacman_grid:
    ...
    def populate_env_with_breadcrumbs(self):
        # breadcrumbs are set in specific locations to enable hyperparameter testing comparisons
        no_breadcrumbs = 0
    ...
    def populate_env_with_random_breadcrumbs(self):
        # breadcrumbs are set in random locations given the stochastic environment
    ...
    def step(self, episode):
        ...
        if Hyper.show_step:
            self.print_curr_grid(f"\nEnvironment for step {self.time_step}")
    ...
    def ghost_step(self, episode):
        ...
        action = self.policy.get_with_available_actions(available_actions)
        new_cell_id = self.get_cell_id_for_action(action)
        reward = self.get_reward(new_cell_id)
        self.total_reward_per_episode += reward
        self.agent_step(new_cell_id)
    ...
    def print_episode_results(self, episodes):
        caption = f"Completed environment after {episodes} episodes and {self.time_step} timesteps, total reward: {self.total_reward_per_episode}"
        print(caption)
